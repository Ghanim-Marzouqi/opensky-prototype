...
@Component({
  ...
  template: `
    <div>
      ...
      <div class="flex items-start justify-between gap-4 mb-6 sm:mb-8">
        <div>
          <h1 class="text-xl sm:text-2xl font-bold text-gray-900">
            {{ lang.currentLanguage() === 'ar' ? 'مرحباً' : 'Hello' }}, {{ userName }}
          </h1>
          <p class="text-accent-500 text-base sm:text-lg font-medium mt-1">
        ...
        <div class="stat-card group">
          <div class="flex items-start justify-between">
            <div class="stat-card-icon bg-gradient-primary-light text-primary-600">
              <ng-icon name="heroUsers" class="text-lg sm:text-2xl"></ng-icon>
            </div>
        ...
        <div class="stat-card group">
          <div class="flex items-start justify-between">
            <div class="stat-card-icon bg-gradient-success-light text-success-600">
              <ng-icon name="heroCheckCircle" class="text-lg sm:text-2xl"></ng-icon>
            </div>
            <span class="text-[10px] sm:text-sm font-semibold text-success-600">94.8%</span>
          </div>
        ...
        <div class="stat-card group">
          <div class="flex items-start justify-between">
            <div class="stat-card-icon bg-gradient-warning-light text-warning-600">
              <ng-icon name="heroCalendarDays" class="text-lg sm:text-2xl"></ng-icon>
            </div>
            <span class="text-[10px] sm:text-sm font-semibold text-warning-600">5.2%</span>
          </div>
        ...
        <div class="stat-card group cursor-pointer" routerLink="/hr/leave/requests">
          <div class="flex items-start justify-between">
            <div class="stat-card-icon bg-gradient-danger-light text-danger-600">
              <ng-icon name="heroClipboardDocumentList" class="text-lg sm:text-2xl"></ng-icon>
            </div>
            @if ((stats()?.pendingRequests || 0) > 0) {
              <span class="badge badge-danger">{{ lang.currentLanguage() === 'ar' ? 'يحتاج' : 'Action' }}</span>
            }
          </div>
              ...
              @for (request of pendingRequests(); track request.id) {
                <div class="p-4 sm:p-5 hover:bg-gray-50/50 transition-colors">
                  <div class="flex items-start gap-3 sm:gap-4">
                    <div class="avatar w-10 h-10 sm:w-11 sm:h-11 text-xs sm:text-sm shrink-0">
                    ...
                    <div class="flex-1 min-w-0">
                      <div class="flex items-center gap-2 flex-wrap">
                        ...
                        <span class="badge text-[10px] sm:text-xs" [class]="getLeaveTypeBadgeClass(request.leaveTypeName)">
                ...
                <div class="p-8 sm:p-10 text-center">
                  <div class="w-14 h-14 sm:w-16 sm:h-16 mx-auto mb-4 rounded-full bg-success-50 text-success-500 flex items-center justify-center">
                    <ng-icon name="heroCheckCircleSolid" class="text-2xl sm:text-3xl"></ng-icon>
                  </div>
          ...
          <div class="card">
            ...
            <div class="divide-y divide-gray-100">
              @for (event of upcomingEvents(); track event.id) {
                <div class="p-3 sm:p-4 flex items-center gap-3 sm:gap-4 hover:bg-gray-50/50 transition-colors">
                  <div class="icon-wrapper w-9 h-9 sm:w-10 sm:h-10" [class]="getEventIconClass(event.type)">
                    <ng-icon [name]="getEventIconName(event.type)" class="text-base sm:text-lg"></ng-icon>
                  </div>
                  <div class="flex-1 min-w-0">
              ...
              @for (activity of recentActivity(); track activity.id) {
                <div class="p-3 sm:p-4">
                  <div class="flex items-start gap-3">
                    <div class="w-2 h-2 sm:w-2.5 sm:h-2.5 mt-1.5 rounded-full shrink-0" [class]="getActivityColor(activity.type)"></div>
                    <div class="flex-1 min-w-0">
                      ...
                      <div class="text-[10px] sm:text-xs text-gray-400 mt-1 sm:mt-1.5 font-medium">
